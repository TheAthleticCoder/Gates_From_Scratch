...
class RNNCell(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim):
        ...
        self.x2h = nn.Linear(embedding_dim, hidden_dim, bias =False)
    ...
    def forward(self, input_word, hx):
        ...
        if hx is None:
            hx = torch.randn(self.hidden_dim).to(device)
        #calculate the hidden state
        insider = self.x2h(embeddings) + self.h2h(hx)
        hy = self.tanh(insider)
        return hy
...
class RNN(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim, n_layers):
        ...
        self.embedding = nn.Embedding.from_pretrained(embedding_matrix)
        self.rnn_cell = RNNCell(vocab_size, embedding_dim, hidden_dim, output_dim)
...
rnn = RNN(vocab_size = len(vocab), embedding_dim = 100, hidden_dim = 300, output_dim = len(vocab), n_layers = 1)
rnn.to(device)
...
test_sentences = brown.sents()[:20000]#print a few examples of the test sentences
...
perplex = []
with open('2020114017-LM2-trainperplexityfin?.txt', 'w') as f:
